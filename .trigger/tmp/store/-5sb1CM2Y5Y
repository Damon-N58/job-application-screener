...
async function generateJWT(options) {
  const { SignJWT } = await import("./esm-NJJK3T7H.mjs");
...
async function randomUUID() {
  const { randomUUID: randomUUID2 } = await import("./crypto.node-LRKJ2KQU.mjs");
  return randomUUID2();
}
__name(randomUUID, "randomUUID");
async function digestSHA256(data) {
  const { subtle: subtle2 } = await import("./crypto.node-LRKJ2KQU.mjs");
...
async function loadSuperJSON() {
  const superjson = await import("./dist-IX3AICS6.mjs");
...
@google-cloud/precise-date/build/src/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-BQEHGKQG.mjs.map
