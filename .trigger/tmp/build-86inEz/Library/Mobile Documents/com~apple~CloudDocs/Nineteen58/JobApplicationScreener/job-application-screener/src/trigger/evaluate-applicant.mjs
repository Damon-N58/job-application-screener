...
// src/trigger/evaluate-applicant.ts
init_esm();
...
var EvaluationSchema = external_exports.object({
  score: external_exports.number().min(0).max(100).describe("Overall match score from 0-100"),
  summary: external_exports.string().describe("2-3 sentence summary of the candidate fit"),
  keyStrengths: external_exports.array(external_exports.string()).describe("3-5 key strengths of the candidate"),
  redFlags: external_exports.array(external_exports.string()).describe("Any concerns or red flags (can be empty)"),
  mustHavesAnalysis: external_exports.array(external_exports.object({
    requirement: external_exports.string(),
    met: external_exports.boolean(),
    notes: external_exports.string().nullable().describe("Notes if applicable, otherwise null")
  })).describe("Analysis of each must-have requirement"),
  niceToHavesAnalysis: external_exports.array(external_exports.object({
    requirement: external_exports.string(),
    met: external_exports.boolean()
  })).describe("Analysis of each nice-to-have requirement"),
  culturalFitScore: external_exports.number().min(0).max(100).describe("Cultural fit score from 0-100")
});
var ApplicantInfoSchema = external_exports.object({
  name: external_exports.string().describe("Full name of the applicant"),
  email: external_exports.string().describe("Email address of the applicant"),
  phone: external_exports.string().nullable().describe("Phone number if found, otherwise null")
});
async function evaluateApplicant(applicant, job) {
...
`;
    ...
    if (applicant.resumeUrl) {
      ...
      try {
        ...
        const { extractTextFromPdfUrl } = await import("../../../../../../../../pdf-parser-FHNO2WWH.mjs");
...
If information is missing or limited, score conservatively and note the gaps in red flags.`;
    const mustHavesList = job.aiPersona.mustHaves?.length ? job.aiPersona.mustHaves.map((r, i) => `${i + 1}. ${r}`).join("\n") : "None specified";
    const niceToHavesList = job.aiPersona.niceToHaves?.length ? job.aiPersona.niceToHaves.map((r, i) => `${i + 1}. ${r}`).join("\n") : "None specified";
...
Provide a comprehensive evaluation of this candidate against all the requirements.`;
    ...
    await supabase.from("applicants").update({ status: newStatus }).eq("id", applicant.id);
    console.log(`📊 Applicant ${applicant.name} marked as ${newStatus}`);
    return evaluation;
  } catch (error) {
    console.error("❌ AI evaluation error:", error);
    console.error("Error details:", error instanceof Error ? error.message : "Unknown error");
    await supabase.from("applicants").update({ status: "incoming" }).eq("id", applicant.id);
    return null;
  }
}
__name(evaluateApplicant, "evaluateApplicant");
async function evaluateApplicantById(applicantId) {
  console.log(`🔄 evaluateApplicantById called for: ${applicantId}`);
  const supabase = getAdminClient();
  const { data: applicant, error: appError } = await supabase.from("applicants").select(`
            id,
            name,
            email,
            email_body,
  ...
  return evaluateApplicant(
    ...
    },
    {
      ...
      aiPersona: job.ai_persona
    }
  );
}
__name(evaluateApplicantById, "evaluateApplicantById");

// src/trigger/evaluate-applicant.ts
var evaluateApplicantTask = task({
...
export {
  evaluateApplicantTask
};
//# sourceMappingURL=evaluate-applicant.mjs.map
